                ...
                <div class="row g-3">
                  ...
                  <div class="col-12">
                    <div class="form-group">
                      <label for="id_booking" class="form-label">Pilih Booking (yang belum lunas)</label>
                      <select id="id_booking" name="id_booking" class="form-select" required>
                        <option value="">-- Pilih Booking --</option>
                        <?php 
                          // hanya tampilkan booking yang status_pembayaran != 'lunas'
                          foreach($booking as $b): 
                            if($b->status_pembayaran != 'lunas'):
                        ?>
                          <option 
                            value="<?= $b->id_booking ?>"
                            data-nama="<?= $b->nama_penghuni ?>"
                            data-kamar="<?= $b->nomor_kamar ?>"
                            data-total="<?= $b->total_harga ?>"
                          >
                            <?= $b->nama_penghuni ?> - Kamar <?= $b->nomor_kamar ?> (<?= ucfirst($b->status_pembayaran) ?>)
                          </option>
                        <?php 
                            endif;
                          endforeach; 
